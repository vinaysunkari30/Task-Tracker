...
class ProjectsPage extends Component{
	...
	getProjectsList = async() => {
		...
		const response = await fetch('http://localhost:3000/projects', options);
	...
	// 	const jsonData = await response.json()
	// 	if(response.ok){
	// 		const formattedList = jsonData.map((eachUser, index) => {
	// 			return {
	// 				id: eachUser.id,
	// 				userId: eachUser.user_id,
	// 				projectName: eachUser.project_name,
	...
	onDeleteProject = async (id) => {
		...
		const response = await fetch(`http://localhost:3000/projects/${id}`, options)
		const jsonData = await response.json();
		if(response.ok){
			await this.getProjectsList();
			this.setState({
